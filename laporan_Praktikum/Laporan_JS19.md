### Praktikum 1 – Setup Jest di Next.js
menginstall dependencies<br>
![alt text](../jobsheet19/gambar_dokumentasi/1.png)<br>
Membuat File Konfigurasi<br>
![alt text](../jobsheet19/gambar_dokumentasi/2.png)<br>
Menambahkan Script di package.json<br>
![alt text](../jobsheet19/gambar_dokumentasi/3.png)<br>

### PRAKTIKUM 2 – Struktur Folder Testing
membuat foler testing<br>
![alt text](../jobsheet19/gambar_dokumentasi/4.png)<br>

### PRAKTIKUM 3 – Testing Halaman About
menambahkan kode di about.spec.tsx<br>
![alt text](../jobsheet19/gambar_dokumentasi/5.png)<br>
Hasil menjalankan testing<br>
![alt text](../jobsheet19/gambar_dokumentasi/6.png)<br>

### PRAKTIKUM 4 – Coverage Report
Mnejalankan npm run test:coverage<br>
![alt text](../jobsheet19/gambar_dokumentasi/7.png)<br>
Muncul folder baru<br>
![alt text](../jobsheet19/gambar_dokumentasi/8.png)<br>
Hasil tampilan coverage<br>
![alt text](../jobsheet19/gambar_dokumentasi/9.png)<br>

### PRAKTIKUM 5 – Konfigurasi Coverage Lengkap
edit kode konfigurasi untuk jest<br>
![alt text](i../jobsheet19/gambar_dokumentasi/10.png)<br>
Hasil saat menjalankan npm run test:coverage<br>
![alt text](../jobsheet19/gambar_dokumentasi/11.png)<br>
Hasil pada tampilan coverage<br>
![alt text](../jobsheet19/gambar_dokumentasi/12.png)<br>

### PRAKTIKUM 6 – Testing dengan getByTestId
edit index pada about<br>
![alt text](../jobsheet19/gambar_dokumentasi/13.png)<br>
Update Testing pada about.spec.tsx<br>
![alt text](../jobsheet19/gambar_dokumentasi/14.png)<br>
run test<br>
![alt text](../jobsheet19/gambar_dokumentasi/15.png)<br>
Jika dibuat salah<br>
![alt text](../jobsheet19/gambar_dokumentasi/16.png)<br>
Hasil run<br>
![alt text](../jobsheet19/gambar_dokumentasi/17.png)<br>
![alt text](../jobsheet19/gambar_dokumentasi/18.png)<br>

### PRAKTIKUM 7 – Testing Page dengan Router (Mocking)
Membuat file product.spec.tsx<br>
![alt text](../jobsheet19/gambar_dokumentasi/19.png)<br>
Menambahkan kode pada product.spec.tsx<br>
![alt text](../jobsheet19/gambar_dokumentasi/20.png)<br>
Hasil :<br>
![alt text](../jobsheet19/gambar_dokumentasi/21.png)<br>
memperbaiki kode di product.spec.tsx untuk solusi Mock Next Router<br>
![alt text](../jobsheet19/gambar_dokumentasi/22.png)<br>
Hasil :<br>
![alt text](../jobsheet19/gambar_dokumentasi/23.png)<br>

### PRAKTIKUM 8 – Menangani Undefined Data
menjalankan test coverage<br>
![alt text](../jobsheet19/gambar_dokumentasi/24.png)<br>
Edit file index.tsx pada folder pages/produk<br>
![alt text](../jobsheet19/gambar_dokumentasi/25.png)<br>
Hasil :<br>
![alt text](../jobsheet19/gambar_dokumentasi/26.png)<br>
perbaikan code pada file index.tsx di view/product<br>
![alt text](../jobsheet19/gambar_dokumentasi/27.png)<br>
Hasil run :<br>
![alt text](../jobsheet19/gambar_dokumentasi/28.png)<br><br>

### Tugas
1. Buat unit test untuk Halaman produk 1 komponen<br>
![alt text](../jobsheet19/gambar_dokumentasi/29.png)<br>
![alt text](../jobsheet19/gambar_dokumentasi/30.png)<br>
Hasil :<br>
![alt text](../jobsheet19/gambar_dokumentasi/31.png)<br><br>

2. Gunakan minimal 1 Snapshot test, 1 toBe(), 1 getByTestId()<br>
![alt text](../jobsheet19/gambar_dokumentasi/32.png)<br><br>

3. Buat coverage minimal 50%<br>
![alt text](../jobsheet19/gambar_dokumentasi/33.png)<br><br>

### Diskusi & Refleksi
1. Mengapa unit testing penting sebelum production?<br>
-> untuk Menemukan Bug sebelum production<br>
2. Mengapa branch coverage sulit mencapai 100%?<br>
-> karena logika di dalam library luar yang dipakai tidak bisa dikontrol sepenuhnya<br>
3. Apa itu mocking?<br>
-> Mocking adalah teknik untuk mengganti komponen asli dengan versi "palsu" atau tiruan selama testing. Tujuannya Agar unit test tetap terisolasi. Supaya unit test tidak gagal hanya karena server API sedang mati atau database sedang lemot.<br>
4. Kapan snapshot test digunakan?<br>
-> Snapshot test digunakan untuk memastikan UI tidak berubah secara tidak sengaja. Snapshot Sangat cocok untuk komponen yang tampilannya sudah stabil.<br>
5. Apakah semua file harus dites?<br>
-> Tidak semua file harus dites<br>
-> file file yang harus dites : file file yang berisi Business Logic, Komponen Reusable, Bagian yang sering error<br>
-> file file yang tidak harus dites : File konfigurasi, File yang hanya berisi tipe data, Komponen UI yang sangat sederhana dan statis contohnya Footer